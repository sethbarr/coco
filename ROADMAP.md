...
- **Encryption honesty** — ✅ plain-language privacy & safety FAQ shipped at `/privacy` (July 28, 2026): states plainly that Coco is NOT E2E encrypted (Coco must read messages to respond), TLS + at-rest is what's real, who-sees-what consent gates, safety scanning disclosure, Stripe identity firewall, honest "deletion not self-serve yet". Keep the page truthful in the same commit as any behavior change. Still open: decide whether to rename the misleading `encryptedContent`/`publicKey` fields.
...
- **Billing** — ✅ *implemented July 28, 2026, flag-off.* To go live: create Stripe products, set `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, `STRIPE_PRICE_MONTHLY`, `STRIPE_PRICE_ANNUAL`, `APP_URL`, and `BILLING_ENABLED=true` on Railway, and point a Stripe webhook at `/api/billing/webhook`. *Decided July 28, 2026:* **per-couple (per-connection) subscription, $19/mo or $149/yr**, free tier = 1 active topic per connection with the full cycle included (the accountability loop is the hook; pay to open a second topic). Never meter messages — cap topics only. Either partner can be the payer; one subscription covers both.
